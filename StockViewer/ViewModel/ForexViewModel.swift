...
//    
//    var forexViewState: ForexViewState = .initial
//    var firstCurrency: String = "1"
//    var secondCurrency: String = ""
//    var firstCurrencyCode: String = "USD"
//    var secondCurrencyCode: String = "AED"
//    var searchTextField = ""
...
@Observable class ForexViewModel {
    ...
    // Observation block tracking the firstCurrency and updating secondCurrency
    func dataw() {
        withObservationTracking {
            print(firstCurrency)
        } onChange: {
            self.calculateCurrencyExchange()
        }
    }

    func readCsv(inputFile: String) {
        self.forexViewState = .loading
        if let filepath = Bundle.main.path(forResource: inputFile, ofType: nil) {
            do {
                let fileContent = try String(contentsOfFile: filepath)
                let lines = fileContent.components(separatedBy: "\n")
                ...
                self.forexViewState = .loaded(data: currencyModel)
            } catch {
                print("Error: \(error.localizedDescription)")
                self.forexViewState = .error(error: error.localizedDescription)
            }
        } else {
            print("\(inputFile) could not be found")
            self.forexViewState = .error(error: "\(inputFile) could not be found")
        }
    }
    ...
    func calculateCurrencyExchange() {
        let doubleFirstCurrency = Double(firstCurrency) ?? 0.0
        let doubleSecondCurrency = Double(secondCurrency) ?? 0.0
        exchangeValue = doubleFirstCurrency * doubleSecondCurrency
    }
}
